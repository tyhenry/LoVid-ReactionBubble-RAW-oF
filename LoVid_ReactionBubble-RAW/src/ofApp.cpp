...
void ofApp::draw() {
	...
	for (int i = 0; i < numBeams; i++) {
		...
		if (timeElapsedSinceBreak < .5) {
			...
			alpha = ofMap(timeElapsedSinceBreak, triggerVidLen * .75, triggerVidLen, 1, 0, true); // fade out
			alpha = pow(alpha, 2);
		}
		if (alpha != 0) {
			ofSetColor(255, alpha * 255);
			//vid.draw(vidX, 0, dW, ofH); // draw video (maybe filter it?)
			filterMgr.draw(vid.getTexture(), vidX, 0, dW, ofH);
		}
		vidX += dW*0.5;
	}
...
void ofApp::beamBreak(int beamIdx, Beam& beam) { // trigger or switch video
	...
	float timeElapsedSincePrevBreak = ofGetElapsedTimef() - beams[beamIdx].lastBreakTime;
	ofLogVerbose("ofApp") << "beam [" << beamIdx << "] broken, last break was " << timeElapsedSincePrevBreak << " seconds ago";

	if (timeElapsedSincePrevBreak > triggerVidLen * .75) { // not 2 triggers in a row
		...
		beam.lastBreakTime = ofGetElapsedTimef();

		ofLogNotice("ofApp") << "beam [" << beamIdx << "] is now vid [" << vidIdx << "]";
	}
